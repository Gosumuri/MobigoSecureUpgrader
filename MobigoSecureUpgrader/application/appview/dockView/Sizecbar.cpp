...
IMPLEMENT_DYNAMIC( CSizingControlBar, CDialogBar )
...
BOOL CSizingControlBar::Create(CWnd* pParentWnd, CSize sizeDefault, UINT nID, DWORD dwStyle) 
{
	...
	// 20031007_01 : jkim  
	//	- "m_dwStyle = dwStyle;"을 수정
	//  - 없으면, Debug Mode에서 Error발생
	m_dwStyle = dwStyle & CBRS_ALL;

//    dwStyle &= ~CBRS_ALL;
    m_dwStyle  |= CCS_NOPARENTALIGN | CCS_NOMOVEY | CCS_NODIVIDER | CCS_NORESIZE;
...
//    if (!CWnd::Create( wndclass, NULL, m_dwStyle, CRect(0,0,0,0), pParentWnd, nID))
//        return FALSE;

	if( !CDialogBar::Create( pParentWnd, IDD_DIALOGBAR, m_dwStyle, nID ) )
		return FALSE;
