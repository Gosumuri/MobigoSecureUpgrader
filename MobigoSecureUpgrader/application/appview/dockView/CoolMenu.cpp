...
{
    DEBUG_LOG( ">> CCoolMenuManager::CCoolMenuManager()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::Destroy()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::DestroyAccel()" );
...
void CCoolMenuManager::Install(CWnd* pWnd)
{
    DEBUG_LOG( ">> CCoolMenuManager::Install()" );

	ASSERT_VALID(pWnd);
	m_pWnd = pWnd;
	HookWindow(m_pWnd);   // install message hook
}

BOOL CCoolMenuManager::IsInstall()
{
    DEBUG_LOG( ">> CCoolMenuManager::IsInstall()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::LoadToolbars()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::LoadToolbar()" );

	// load bitmap
	HBITMAP hbmToolbar = PxLib::LoadSysColorBitmap(nIDToolbar);
	if (!hbmToolbar) {
		ERROR_LOG( "++   [ERROR] Can't load bitmap for toolbar %d", nIDToolbar);
		return FALSE;
	}
	CBitmap bmToolbar;
	...
	if ((hInst= AfxFindResourceHandle(lpResName, RT_TOOLBAR)) == NULL ||
		...
		ERROR_LOG( "++   [ERROR] Can't load toolbar %d", nIDToolbar);
		return FALSE;
	}
	...
	if (m_szBitmap.cx==0) {
		...
		ERROR_LOG( "++   [ERROR] Toolbar %d button size differs", nIDToolbar);
		return FALSE;
	}
	...
	for (int i = 0; i < ptbd->wItemCount; i++) {
		...
		if (nID > 0) {
			if (GetButtonIndex(nID) >= 0) {
				ERROR_LOG( "++   [ERROR]  Duplicate button ID %d ignored", nID);
			} else {
...
{
    DEBUG_LOG( ">> CCoolMenuManager::Refresh()" );
...
{
    //-DEBUG_LOG( ">> CCoolMenuManager::GetMenuFont()" );
...
{
    //-DEBUG_LOG( ">> CCoolMenuManager::OnMeasureItem()" );
...
{
    //-DEBUG_LOG( ">> CCoolMenuManager::OnDrawItem()" );
...
void CCoolMenuManager::DrawMenuText(CDC& dc, CRect rc, CString text,
	COLORREF color)
{
    DEBUG_LOG( ">> CCoolMenuManager::DrawMenuText()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::Draw3DCheckmark()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::OnInitMenuPopup()" );

#if 0 // dialog만 있는 application에서도 사용가능하도록 하기 위해서
	if (m_bAutoAccel) {
		// check for new accels. If ASSERT bombs, you forgot to call Install.
		ASSERT_VALID(m_pWnd);
		HACCEL hAccel = m_pWnd->GetDefaultAccelerator(); // compile error 
		if (hAccel != m_hAccel)
			LoadAccel(hAccel);
	}
#endif
...
{
    DEBUG_LOG( ">> CCoolMenuManager::LoadAccel()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::ConvertMenu()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::OnMenuChar()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::OnMenuSelect()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::AppendAccelName()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::FixMFCDotBitmap()" );
...
{
    DEBUG_LOG( ">> CCoolMenuManager::GetMFCDotBitmap()" );
