...
CUtilHttpPost::CUtilHttpPost(void)
{
...
CUtilHttpPost::RequestMethod CUtilHttpPost::GetMethod(int nMethod)
{	
	return nMethod<=0 ? CUtilHttpPost::RequestGetMethod : static_cast<CUtilHttpPost::RequestMethod>(nMethod);
}

CUtilHttpPost::TypePostArgument CUtilHttpPost::GetPostArgumentType(int nType)
{
	return nType<=0 ? CUtilHttpPost::TypeNormal : static_cast<CUtilHttpPost::TypePostArgument>(nType);
}

BOOL CUtilHttpPost::Connect(LPCTSTR szAddress, LPCTSTR szAgent, unsigned short nPort, LPCTSTR szUserAccount, LPCTSTR szPassword){
...
BOOL CUtilHttpPost::Request(LPCTSTR szURL, int nMethod, LPCTSTR szAgent){
...
BOOL CUtilHttpPost::RequestOfURI(LPCTSTR szURI, int nMethod){

	BOOL bReturn=TRUE;

	try{
		switch(nMethod){
		case	CUtilHttpPost::RequestGetMethod:
		default:
			bReturn=RequestGet(szURI);
			break;
		case	CUtilHttpPost::RequestPostMethod:		
			bReturn=RequestPost(szURI);
			break;
		case	CUtilHttpPost::RequestPostMethodMultiPartsFormData:
...
BOOL CUtilHttpPost::RequestGet(LPCTSTR szURI)
{
...
BOOL CUtilHttpPost::RequestPost(LPCTSTR szURI ){

	CONST TCHAR     *szAcceptType=__HTTP_ACCEPT_TYPE;
	memset( m_szPostArguments, 0, sizeof(TCHAR)*__SIZE_BUFFER );
	LPCTSTR			szContentType=TEXT("Content-Type: application/x-www-form-urlencoded\r\n");

	DWORD dwPostArgumentsLegnth=GetPostArguments(m_szPostArguments, __SIZE_BUFFER);
...
*/
	...
	if(!::HttpSendRequest( _hHTTPRequest,	// handle by returned HttpOpenRequest
									...
									(LPVOID)m_szPostArguments, // additional data in HTTP Post or HTTP Put
									_tcslen(m_szPostArguments)) // additional data length
									){
		_dwError=::GetLastError();
/*
#ifdef	_DEBUG
...
BOOL CUtilHttpPost::RequestPostMultiPartsFormData(LPCTSTR szURI){
...
DWORD CUtilHttpPost::ResponseOfBytes(PBYTE pBuffer, DWORD dwSize){
...
BOOL CUtilHttpPost::Response(PBYTE pHeaderBuffer, DWORD dwHeaderBufferLength, PBYTE pBuffer, DWORD dwBufferLength, DWORD &dwResultSize){
...
VOID CUtilHttpPost::AddPostArguments(LPCTSTR szName, LPCTSTR szValue, BOOL bBinary){
	...
	if(!bBinary)
		arg.dwType = CUtilHttpPost::TypeNormal;
	else
		arg.dwType = CUtilHttpPost::TypeBinary;	
...
VOID CUtilHttpPost::AddPostArguments(LPCTSTR szName, DWORD nValue){
...
DWORD CUtilHttpPost::GetPostArguments(LPTSTR szArguments, DWORD dwLength){
...
VOID CUtilHttpPost::InitilizePostArguments(){
	_vArguments.clear();
	return;
}

VOID CUtilHttpPost::FreeMultiPartsFormData(PBYTE &pBuffer){
...
DWORD CUtilHttpPost::AllocMultiPartsFormData(PBYTE &pInBuffer, LPCTSTR szBoundary){
...
DWORD CUtilHttpPost::GetMultiPartsFormDataLength(){
	std::vector<GenericHTTPArgument>::iterator itArgv;
...
LPCTSTR CUtilHttpPost::GetContentType(LPCTSTR szName){
...
DWORD CUtilHttpPost::GetLastError(){

	return _dwError;
}

VOID CUtilHttpPost::ParseURL(LPCTSTR szURL, LPTSTR szProtocol, LPTSTR szAddress, DWORD &dwPort, LPTSTR szURI){
...
LPCTSTR CUtilHttpPost::QueryHTTPResponseHeader()
{
	return _szHTTPResponseHeader;
}

LPCTSTR CUtilHttpPost::QueryHTTPResponse()
{
